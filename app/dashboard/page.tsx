...
export default function UserDashboard() {
    ...
    if (savedMems) {
      ...
      setTimeout(() => {
        ...
        if (parsedMems.length > 0) {
          const first = parsedMems[0];
          setSelectedId(first.id);
          setEditName(first.name);
          setEditBirth(first.birthDate);
          setEditDeath(first.deathDate);
          setEditBio(first.biography);
          setEditIsPrivate(first.isPrivate);
          setEditPassword(first.password || "");
        }
      }, 0);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#FCFBFA] font-sans text-[#111111] text-sm relative overflow-x-hidden">
      ...
      {/* Header Dashboard */}
      <header className="sticky top-0 z-40 bg-white border-b border-stone-200/50 px-8 py-4 flex justify-between items-center shadow-2xs">
      ...
      <div className="max-w-7xl mx-auto px-8 py-12 grid lg:grid-cols-4 gap-8">
        ...
        <main className="lg:col-span-3 space-y-8 text-left">
          ...
          {activeTab === "list" && (
            <div className="space-y-8">
              <div className="bg-white/80 backdrop-blur-sm border border-stone-200/60 p-8 rounded-2xl shadow-2xs space-y-6">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {memorials.map((mem) => (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center pt-3 border-t border-stone-100 text-base">
                        <button
              ...
              <div className="bg-white/80 backdrop-blur-sm border border-stone-200/60 p-8 rounded-2xl shadow-2xs">
                        ...
                        {accessRequests.map((req, idx) => (
                          <div key={idx} className="p-4 rounded-xl border border-stone-200/60 bg-white shadow-xs">
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => handleAcceptRequest(req.email, req.relation)} className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold uppercase tracking-wider rounded">Aceptar</button>
                              <button onClick={() => handleRejectRequest(req.email)} className="flex-1 py-1.5 bg-stone-100 hover:bg-red-50 text-neutral-600 hover:text-red-600 text-xs font-bold uppercase tracking-wider rounded transition-colors">Rechazar</button>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      {invitedMembers.map((item, idx) => (
                        <div key={idx} className="p-3 rounded-xl border border-stone-200/60 bg-stone-50/40 flex justify-between items-center">
                          <div>
          ...
          {activeTab === "qr" && activeMemorial && (
            <div className="max-w-md mx-auto">
              <QrCodeGenerator 
                url={`http://localhost:3000/memorial/${activeMemorial.slug}`} 
